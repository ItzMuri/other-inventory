...
local function OpenVending()
    ...
    TriggerServerEvent("inventory:server:OpenInventory", nil, "shop", "Vendingshop_"..math.random(1, 99), ShopItems)
end
			...
			options = {
				{
					...
					action = function()
						TriggerServerEvent("inventory:server:OpenInventory", nil, "drop", index)
					end,
				}
			},
			distance = 2.5,
...
RegisterNetEvent('inventory:client:craftTarget',function()
    ...
    TriggerServerEvent("inventory:server:OpenInventory", nil, "crafting", math.random(1, 99), crafting)
end)
    ...
    if not isCrafting and not inInventory then
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
            ...
            if CurrentVehicle then -- Trunk
                ...
                TriggerServerEvent("inventory:server:OpenInventory", nil, "trunk", CurrentVehicle, other)
                OpenTrunk()
            elseif CurrentGlovebox then
                TriggerServerEvent("inventory:server:OpenInventory", nil, "glovebox", CurrentGlovebox)
            elseif CurrentDrop ~= 0 then
                TriggerServerEvent("inventory:server:OpenInventory", nil, "drop", CurrentDrop)
            elseif VendingMachine then
                ...
                openAnim()
                TriggerServerEvent("inventory:server:OpenInventory",  nil)
            end
        end
    end
end, false)
            ...
            if distance < 10 then
                if distance < 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        TriggerServerEvent("inventory:server:OpenInventory", nil, "attachment_crafting", math.random(1, 99), crafting)
                        sleep = 100
                    end
                end
            end
